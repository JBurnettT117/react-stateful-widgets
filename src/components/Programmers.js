...
export default function Programmers() {
  const [listOfPioneers, setListOfPioneers] = useState(listOfAwesome);
  const [currentFeature, setCurrentFeature] = useState(null);
  // We'll have to use the state hook twice, as we need two slices of state.
  // The programmers list on the one hand, and the id of the featured programmer on the other.

  const getNameOfFeatured = () => {
    return listOfPioneers[currentFeature - 1].name;
    // Leave this for last!
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: currentFeature === null?'royalblue':"gold", // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfPioneers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ dev.id === currentFeature? setCurrentFeature(null): setCurrentFeature(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
